...
Uso:
    ...
    python -m src.explain --run densenet-oasis3-adni-mni --dataset oasis3_adni \\
        --preprocess-variant mni --source-filter oasis3 --classes 2
"""
...
def _save_figure(
    ...
    # Volumen (D, H, W) registrado a MNI152 (orden RAS): 1.er eje = sagital (L-R),
    # 2.o eje = coronal (A-P), 3.er eje = axial (I-S). Se etiqueta cada plano con su
    # eje real y se mantiene el orden axial-coronal-sagital del caption.
    slices = {
        "Axial (W/2)":    (img_np[:, :, w_mid],  heatmap[:, :, w_mid]),
        "Coronal (H/2)":  (img_np[:, h_mid, :],  heatmap[:, h_mid, :]),
        "Sagital (D/2)":  (img_np[d_mid, :, :],  heatmap[d_mid, :, :]),
    }
...
def explain_model(
    ...
    Args:
        ...
        dataset:            Identificador del dataset (oasis1, oasis3, adni, oasis3_adni, ...).
        ...
        source_filter:      Filtrar filas del split fusionado por columna source (oasis3/adni).
    """
    ...
    for batch in loader:
        ...
        # Predicción sin gradientes — decodificación unificada (CORAL / estándar)
        with torch.no_grad():
            logits = model(image, clinical) if use_clinical else model(image)
            pred_label = int(decode_preds(logits, model).item())
